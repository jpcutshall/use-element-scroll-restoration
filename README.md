...
```bash
npm install use-element-scroll-restoration
```

### Usage

Here's how you can use the `useElementScrollRestoration` hook in your React components:

```jsx
import React from "react";
import { useElementScrollRestoration } from "use-element-scroll-restoration";

function ScrollRestorationExample() {
  const { ref, setScroll } = useElementScrollRestoration("exampleKey", {
    debounceTime: 200,
...
#### `useElementScrollRestoration(key: string, options?: ScrollRestorationOptions)`
